...
class BatchGenerator:
    ...
    def generate(self,
        ...
        while True:
            ...
            for filename in self.Mask_filenames[current:current+batch_size]:
                if not os.path.exists(filename):
                    print(filename+ " is not exists")
                with Image.open(filename) as img:
                    batch_Y.append(np.array(img))

            this_filenames = self.Img_filenames[current:current+batch_size]  #当前batch的样本名
            if train==False:
...
if __name__=='__main__':
    batch_generater = BatchGenerator()
    batch_generater.parse_xml(images_paths=[r"E:\data_base\breast\main_2000_jpg"],
                              mask_paths=[r"E:\data_base\breast\main_2000_mask_bmp"],
                              image_set_paths=[r"E:\data_base\breast\train.txt"])
    generator = batch_generater.generate(brightness = [0.5,0.7,0.5],
                                         resize = [224, 224],
                                         zoom_range=[0.96,1.22],
                                         rotation_range = 25,
